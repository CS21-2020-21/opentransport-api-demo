...
def populate():
    modes = [{'id':1, 'short_desc':'train', 'long_desc':'monorail, freight train etc'},
            {'id':2, 'short_desc':'bus', 'long_desc':'includes double deckers'},
            {'id':3, 'short_desc':'ferry', 'long_desc':'includes car and foot passengers'},
            {'id':4, 'short_desc':'cable car', 'long_desc':'alternative name funicular railway'},
            ]
    ...
    operators = [{'id':'32H', 'name':'PSD Ferries','homepage':'www.psdferries.com', 'api_url':'www.pythonanywhere.psdferries.com', 'default_language':'en', 'phone':'01413267899', 'email':'psdferries@gmail.com' },
                 {'id':'78Y', 'name':'PSD Trains','homepage':'www.psdtrains.com', 'api_url':'www.pythonanywhere.psdtrains.com', 'default_language':'en', 'phone':'01413267896', 'email':'psdtrains@gmail.com' },
                 {'id':'23J', 'name':'PSD Buses','homepage':'www.psdBuses.com', 'api_url':'www.pythonanywhere.psdbuses.com', 'default_language':'en', 'phone':'01413267893', 'email':'psdbuses@gmail.com' },
                 {'id':'98J', 'name':'PSD Cable Cars','homepage':'www.psdCableCars.com', 'api_url':'www.pythonanywhere.psdcablecars.com', 'default_language':'en', 'phone':'01413267891', 'email':'psdcablecars@gmail.com' },
                ]
    ...
    customers = [{'id':1, 'name':'Michael Palin', 'email':'palin@gmail.com'},
                {'id':2, 'name':'Christopher Columbus', 'email':'columbus@gmail.com'},
                {'id':3, 'name':'David Attenborough', 'email':'da@hotmail.com'},
                {'id':4, 'name':'Bear Grylls', 'email':'bear@yahoo.com'},
                {'id':5, 'name':'Tim Peake', 'email':'peake@gmail.com'},
                ]
    ...
    vehicles = [{'included':'yes', 'reference':'BA20 FGD', 'vehicle_type':'bus', 'condition':'brand new' },
                {'included':'no', 'reference':'6732', 'vehicle_type':'train', 'condition':'old' },
                {'included':'yes', 'reference':'DR19 JKD', 'vehicle_type':'bus', 'condition':'nearly new' },
                {'included':'no', 'reference':'789GH43', 'vehicle_type':'ferry', 'condition':'rusty and needing repairs' },
                {'included':'yes', 'reference':'2F78HJ6', 'vehicle_type':'cable car', 'condition':'brand new' },
            ]
    ...
    usages = [{'id':'123', 'customer':Customer.objects.get(customer_id=2), 'mode':Mode.objects.get(id=3), 
                'operator':Operator.objects.get(id='32H'), 'reference':Reference.objects.get(id=1),
                ...
                {'id':'124', 'customer':Customer.objects.get(customer_id=3), 'mode':Mode.objects.get(id=2), 
                'operator':Operator.objects.get(id='78Y'), 'reference':Reference.objects.get(id=2),
    ...
    concessions = [{'id':'124', 'customer':Customer.objects.get(customer_id=3), 'mode':Mode.objects.get(id=4), 
                'operator':Operator.objects.get(id='78Y'), 'name':'family concession', 'price':Monetary_Value.objects.get(money_id=1),
                ...
                {'id':'156', 'customer':Customer.objects.get(customer_id=1), 'mode':Mode.objects.get(id=2), 
                'operator':Operator.objects.get(id='78Y'), 'name':'winter sale card', 'price':Monetary_Value.objects.get(money_id=3),
                ...
                {'id':'194', 'customer':Customer.objects.get(customer_id=5), 'mode':Mode.objects.get(id=1), 
                'operator':Operator.objects.get(id='78Y'), 'name':'summer sale card', 'price':Monetary_Value.objects.get(money_id=4),
    ...
    purchases = [{'id':'983', 'customer':Customer.objects.get(customer_id=5), 'mode':Mode.objects.get(id=2), 
                'operator':Operator.objects.get(id='23J'), 
                'travel_class':'coach', 'booking_date_time':'2020-12-07T11:33:11Z', 'transaction':Transaction.objects.get(reference='432'),
                'account_balance':AccountBalance.objects.get(account_id=6), 'agent':'PSD train operators', 
                'passenger_number':2, 'passenger_type':'two adults', 'vehicle':Vehicle.objects.get(reference='6732'),
                ...
                {'id':'678', 'customer':Customer.objects.get(customer_id=4), 'mode':Mode.objects.get(id=1), 
                'operator':Operator.objects.get(id='98J'), 
                'travel_class':'first', 'booking_date_time':'2021-01-07T11:33:11Z', 'transaction':Transaction.objects.get(reference='432'),
                'account_balance':AccountBalance.objects.get(account_id=2), 'agent':'PSD ferry operators', 
                'passenger_number':3, 'passenger_type':'two adults, one child', 'vehicle':Vehicle.objects.get(reference='789GH43'),
                ...
                {'id':'681', 'customer':Customer.objects.get(customer_id=1), 'mode':Mode.objects.get(id=4), 
                'operator':Operator.objects.get(id='98J'), 
                'travel_class':'first', 'booking_date_time':'2020-01-07T11:33:11Z', 'transaction':Transaction.objects.get(reference='986'),
                'account_balance':AccountBalance.objects.get(account_id=5), 'agent':'PSD bus operators', 
                'passenger_number':1, 'passenger_type':'one adult', 'vehicle':Vehicle.objects.get(reference='BA20 FGD'),
...
def add_customer(customer):
    person = Customer.objects.get_or_create(customer_id=customer['id'], name=customer['name'], email=customer['email'])[0]
    person.save()
    return person
