    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if code == "123456":
                ...
                operator_to_url_mapping = {operator['item_metadata'][0]['val']:operator['item_metadata'][1]['val'] for operator in operators_json}
                operator_url = operator_to_url_mapping[operator] + 'api-token-auth/'
                headers = {}
                data = {'username':username, 'password':'12345678abc'}
                response = requests.post(operator_url, data=data, headers=headers)

                token = response.json()['token']

                linked_account = LinkedAccount.objects.get_or_create(operator=operator, email=email, customer=customer, username=username, token=token)
...
def show_linked_account_purchases(request, id_slug):
    ...
    try:
        ...
        linked_account = LinkedAccount.objects.get(id=int(id_slug))
        ...
        for operator in all_operators:
            if operator['item_metadata'][0]['val']==linked_account.operator:
                ...
                headers = {'Authorization':'Token '+linked_account.token}
                purchases = requests.get(url=purchase_url, headers=headers).json()
                purchase_list = []
    ...
    try:
        ...
        for operator in all_operators:
            if operator['item_metadata'][0]['val']==linked_account.operator:
                ...
                headers = {'Authorization':'Token '+linked_account.token}
                concessions = requests.get(url=concession_url, headers=headers).json()
                concession_list = []
    ...
    try:
        ...
        for operator in all_operators:
            if operator['item_metadata'][0]['val']==linked_account.operator:
                ...
                headers = {'Authorization':'Token '+linked_account.token}
                usages = requests.get(url=usage_url, headers=headers).json()
                usage_list = []
                for usage in usages:
                    details = {}
